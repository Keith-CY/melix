...
- Reduce repeated q4 affine scale/bias metadata loads in the fused q4 decode attention kernel by loading scale/bias on one leader lane per q4 group and distributing it to the group's packed-word lanes with `simd_shuffle`.
