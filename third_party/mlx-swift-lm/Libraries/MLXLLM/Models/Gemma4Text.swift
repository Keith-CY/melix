...
private class Gemma4TextModelInner: Module {
    ...
    func callAsFunction(
        ...
        for (idx, layer) in layers.enumerated() {
            ...
            if isAssistant {
                fatalError("Gemma4 assistant MTP path is not supported by this MLXLMCommon version")
            } else {
    ...
    public func newCache(parameters: GenerateParameters?) -> [any KVCache] {
        ...
        for i in 0 ..< firstKvShared {
            if config.layerTypes[i] == "full_attention" {
                ...
                cache.step = max(1024, config.slidingWindow)
                caches.append(cache)
            } else {
