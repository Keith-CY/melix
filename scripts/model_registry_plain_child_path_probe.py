...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        weight_model_dir = Path(tmpdir) / "artifact-weight-probe"
        weight_model_dir.mkdir(parents=True, exist_ok=True)
        (weight_model_dir / "config.json").write_text(
            json.dumps(
                {
                    "model_type": "bert",
                    "hidden_size": 4,
        ...
        (weight_model_dir / "tokenizer.json").write_text("{}", encoding="utf-8")
        (weight_model_dir / "model.safetensors").write_bytes(b"weights")
        (weight_model_dir / "notes.safetensors.txt").write_text(
            "ignored",
            encoding="utf-8",
        )
        weight_pooling_config_path = weight_model_dir / "1_Pooling" / "config.json"
        weight_pooling_config_path.parent.mkdir()
        weight_pooling_config_path.write_text(
            json.dumps(
                {
                    "pooling_mode_mean_tokens": True,
                    "word_embedding_dimension": 4,
        ...
        (weight_model_dir / "modules.json").write_text(
            json.dumps(
                [
                    {
                        "idx": 0,
        ...
        for _ in range(sample_count):
            ...
            def tracking_eq(
                self: Path,
                other: object,
            ) -> bool:  # pragma: no cover - optimized path should not call this
            ...
            def tracking_glob(self: Path, pattern: str):
                nonlocal module_glob_calls, weight_glob_calls
                if self == embedding_model_dir:  # pragma: no cover - optimized head should not call Path.glob
                    module_glob_calls += 1
                if (
                    self == weight_model_dir
                    and pattern == "*.safetensors"
                ):  # pragma: no cover - optimized head should not call Path.glob
                    weight_glob_calls += 1
            ...
            def tracking_scandir(path: str):
                nonlocal module_scandir_calls, weight_scandir_calls
            ...
            try:
                ...
                manifests, plain_scans, hf_repos = (
                    WorkerModelCatalog._scan_registry_root_tree_with_hf_repos(root)
                )
                module_paths = model_catalog._artifact_embedding_module_paths(embedding_model_dir)
                embedding_metadata = model_catalog._artifact_embedding_metadata(
                    weight_model_dir,
                    json.loads(
                        (weight_model_dir / "config.json").read_text(
                            encoding="utf-8",
                        )
                    ),
                    json_cache={},
            ...
            if module_paths != (pooling_config_path, normalize_config_path):
                raise SystemExit(
                    f"unexpected embedding module paths: {module_paths!r}"
                )  # pragma: no cover
            if not embedding_metadata:
                raise SystemExit("expected artifact embedding metadata")  # pragma: no cover

            if hf_repos:
                raise SystemExit(f"unexpected hf repos: {hf_repos!r}")
            if len(plain_scans) != model_count:  # pragma: no cover
                raise SystemExit(
                    f"unexpected plain scan count: {len(plain_scans)} != {model_count}"
                )
            if len(manifests) != model_count:  # pragma: no cover
                raise SystemExit(
                    f"unexpected manifest count: {len(manifests)} != {model_count}"
                )
        ...
        json.dumps(
            {
                ...
                "root_plain_child_path_joins_mean": round(
                    statistics.fmean(root_plain_child_join_samples),
                    6,
                ),
                "root_identity_comparisons_mean": round(
                    statistics.fmean(root_identity_comparison_samples),
                    6,
                ),
                "plain_scan_count_mean": round(
                    statistics.fmean(plain_scan_count_samples),
                    6,
                ),
                "manifest_count_mean": round(statistics.fmean(manifest_count_samples), 6),
                "module_path_glob_calls_mean": round(
                    statistics.fmean(module_glob_call_samples),
                    6,
                ),
                "module_path_scandir_calls_mean": round(
                    statistics.fmean(module_scandir_call_samples),
                    6,
                ),
                "artifact_weight_glob_calls_mean": round(
                    statistics.fmean(weight_glob_call_samples),
                    6,
                ),
