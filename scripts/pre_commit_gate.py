...
def performance_probe_tmpdir_env(env: Mapping[str, str] | None = None) -> dict[str, str]:
    env = os.environ if env is None else env
    tmpdir = env.get("TMPDIR", "").strip()
    if not tmpdir:
        return {}
    return {"TMPDIR": os.fspath(Path(tmpdir).expanduser().resolve())}
...
def run_performance_report(root: Path, changed_files: list[str], *, base_ref: str | None = "HEAD") -> PerformanceOutcome:
    ...
    if selected_probe_count:
        with tempfile.TemporaryDirectory(prefix="melix-pre-commit-probes-") as probe_temp:
            ...
            probe_env = {
                **performance_probe_git_identity_env(root),
                **performance_probe_tmpdir_env(),
            }
            if base_ref is not None:
