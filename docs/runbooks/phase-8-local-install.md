...
Melix-managed Hugging Face downloads write model bytes directly to the resolved Hugging Face cache root. By default this is `~/.cache/huggingface/hub`; operators can override it with request metadata `melix.hf_cache_root` or `hf_cache_root`, or through `HUGGINGFACE_HUB_CACHE` / `HF_HOME` when no request-level root is supplied. Melix passes the resolved root as `snapshot_download(cache_dir=...)`, records it as `melix.effective_hf_cache_root`, and reports the real snapshot directory, for example `<cache-root>/models--mlx-community--Qwen3-0.6B-4bit/snapshots/<snapshot-id>`. New downloads do not create descriptor directories under `MELIX_MANAGED_MODEL_ROOT`.

Registry scanning reads user-configured model roots first and then appends the effective Hugging Face cache root: `HUGGINGFACE_HUB_CACHE`, `<HF_HOME>/hub`, or the default `~/.cache/huggingface/hub` when no environment cache root is configured. Missing configured cache roots are reported as inaccessible registry roots instead of being silently replaced by the default. It discovers Hugging Face cache snapshots under `models--<org>--<repo>/snapshots/<snapshot-id>` and plain local MLX directories that contain `config.json` plus weights. Only models with explicit MLX compatibility signals are shown. Non-MLX Transformers repositories, ambiguous local folders, unreadable metadata, and Hugging Face `blobs` payloads are ignored. If a cache snapshot is removed and the registry is rescanned, that model disappears from `melix model list` and `/v1/models`; it does not remain as a missing-cache descriptor entry.
