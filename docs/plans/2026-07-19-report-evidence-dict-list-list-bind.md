# Report evidence gate dict-list list-type binding
...
identity for already-valid exact lists and list subclasses.
...
The probe reports `dict_list_elapsed_ms_mean`, `dict_list_rows_per_call`, and
`dict_list_identity_hits`; those metrics are the primary evidence for this
list-type binding fast path.

## Implementation plan

1. Preserve `_dict_list(...)` semantics for exact `list`, list subclasses, exact
   `dict` rows, dict subclasses, mixed rows, and non-list inputs.
2. Bind the `list` type once per `_dict_list(...)` call and reuse the already
   local `isinstance` helper for the list-subclass fallback guard, avoiding two
   global lookups on the hot normalizer entry path.
