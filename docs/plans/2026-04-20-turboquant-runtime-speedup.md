...
**Current evidence:** former `docs/metrics/phase2-active-kv-qwen35-hybrid-stability-summary.json`, now archived in issue [#46](https://github.com/Keith-CY/melix/issues/46), shows five routed Qwen3.5 runs with `worker_tps_overhead_pct` min/median/mean/max of `5.71 / 10.26 / 9.722 / 12.82`. The route is stable, but `decode_model_eval_sync_total_us` dominates the profile (`3.57s-4.13s`), while fused attention totals are only `216ms-229ms` across `1134` calls. The next optimization must therefore be small and measured against real-model noise.
...
- `scaleBiasLoadLaneCount` equals the q4 group count for the fused launch plan, so `headDimension = 128` and `groupSize = 64` reads key/value scale-bias metadata from 2 group-leader lanes instead of all 16 active packed-word lanes, then distributes it with simdgroup shuffle.
...
- [x] **Task 5: Re-audit referenced repos for another small fused-route optimization**
  - Compare Melix with current `AtomicBot-ai/Atomic-Chat`, `jundot/omlx`, and `jjang-ai/vmlx` repository code.
  - Keep broad batch, disk, and shape-repair ideas out of this slice unless they directly improve the proven one-token fused decode path.
  - Add RED coverage for any new launch-plan or kernel-work invariant before changing vendored Swift code.
  - If a local kernel optimization lands, run the targeted fused attention tests and keep unsupported shapes on the fallback path.

## Evidence

- Pre-optimization JSON: former `docs/metrics/phase2-active-kv-qwen35-turboquant-speedup-preopt.json`, archived in issue [#46](https://github.com/Keith-CY/melix/issues/46).
- Post-optimization JSON: former `docs/metrics/phase2-active-kv-qwen35-turboquant-speedup-postopt.json`, archived in issue [#46](https://github.com/Keith-CY/melix/issues/46).
...
- Stability summary JSON: former `docs/metrics/phase2-active-kv-qwen35-turboquant-speedup-stability-summary.json`, archived in issue [#46](https://github.com/Keith-CY/melix/issues/46).
...
- The May 4 repo re-audit found that current oMLX still routes only single-token decode into TurboQuant fused attention and keeps batching as merge/extract/extend/filter/left-padding state management; vMLX's relevant ideas are TQ-native disk serialization, codebook weight caching, and quantized-weight shape repair; Atomic-Chat did not expose a directly reusable TurboQuant fused-KV implementation in its current MLX tree.
- The resulting local kernel slice keeps Melix's single-token fused route and reduces repeated q4 affine scale/bias metadata loads inside a q4 group. For the Qwen3.5-style launch plan (`headDimension = 128`, `groupSize = 64`), `scaleBiasLoadLaneCount` is 2 while `scoreReductionLaneCount` remains 16; group leader lanes load key/value scale-bias metadata and distribute it to packed-word lanes in the same group with `simd_shuffle`.
- No new real-model Phase 2 JSON was captured for this May 4 slice; treat it as a deterministic kernel-work reduction plus targeted correctness verification, not a fresh throughput gate claim.

## Verification

- `jq empty docs/metrics/phase2-active-kv-qwen35-turboquant-speedup-preopt.json docs/metrics/phase2-active-kv-qwen35-turboquant-speedup-postopt.json`; this applied to the local raw JSON files before they were archived in issue [#46](https://github.com/Keith-CY/melix/issues/46).
- `make phase2-metrics PHASE2_METRICS_ARGS='--input-json docs/metrics/phase2-active-kv-qwen35-turboquant-speedup-postopt.json --json --require-fused-turboquant'`; this applied to the local post-optimization JSON before archive.
...
- `git diff --check`
- `swift test --package-path services/mlx-text-worker-swift --filter 'WorkerScaffoldTests/testVendoredFusedQ4AttentionLaunchPlanTracksScaleBiasLoadReduction|WorkerScaffoldTests/testVendoredFusedQ4AttentionLaunchPlanRejectsInvalidPackedScaleBiasGroups'`: RED failed first because `TurboQuantFusedAttentionLaunchPlan` did not expose the computed `usesReducedScaleBiasLoads` invariant.
- `swift test --package-path services/mlx-text-worker-swift --filter 'WorkerScaffoldTests/testVendoredFusedQ4AttentionLaunchPlanUsesOnlineSoftmaxAcrossValueLanes|WorkerScaffoldTests/testVendoredFusedQ4AttentionLaunchPlanTracksScaleBiasLoadReduction|WorkerScaffoldTests/testVendoredFusedQ4AttentionLaunchPlanRejectsInvalidPackedScaleBiasGroups|WorkerScaffoldTests/testVendoredFusedQ4AttentionMatchesQuantizedReferenceForDecodeGQA|WorkerScaffoldTests/testVendoredFusedQ4AttentionPreservesQueryDTypeForDecodeGQA|WorkerScaffoldTests/testVendoredFusedQ4AttentionMatchesQuantizedReferenceForDecodeHeadDimension128|WorkerScaffoldTests/testAttentionWithCacheUpdateUsesFusedQuantizedStorageWithoutMaterializingForDecode|WorkerScaffoldTests/testTurboQuantRuntimeRouteReportsRoutedAfterFusedAttentionDispatch|WorkerScaffoldTests/testTurboQuantRuntimeRouteReportsRoutedFromDispatchEvidenceBeforeStateRecheck|WorkerScaffoldTests/testTurboQuantActiveKVUsesVendoredRuntimeWithoutProbe|WorkerScaffoldTests/testQuantizedKVCacheUsesFusedDecodeQuantizerWhenExplicitlyEnabledForSingleTokenAffineQ4'`: 11 tests passed.
- `swift test --package-path services/mlx-text-worker-swift --enable-code-coverage --filter 'WorkerScaffoldTests/testVendoredFusedQ4AttentionLaunchPlanUsesOnlineSoftmaxAcrossValueLanes|WorkerScaffoldTests/testVendoredFusedQ4AttentionLaunchPlanTracksScaleBiasLoadReduction|WorkerScaffoldTests/testVendoredFusedQ4AttentionLaunchPlanRejectsInvalidPackedScaleBiasGroups|WorkerScaffoldTests/testVendoredFusedQ4AttentionMatchesQuantizedReferenceForDecodeGQA|WorkerScaffoldTests/testVendoredFusedQ4AttentionPreservesQueryDTypeForDecodeGQA|WorkerScaffoldTests/testVendoredFusedQ4AttentionMatchesQuantizedReferenceForDecodeHeadDimension128|WorkerScaffoldTests/testAttentionWithCacheUpdateUsesFusedQuantizedStorageWithoutMaterializingForDecode|WorkerScaffoldTests/testTurboQuantRuntimeRouteReportsRoutedAfterFusedAttentionDispatch|WorkerScaffoldTests/testTurboQuantRuntimeRouteReportsRoutedFromDispatchEvidenceBeforeStateRecheck|WorkerScaffoldTests/testTurboQuantActiveKVUsesVendoredRuntimeWithoutProbe|WorkerScaffoldTests/testQuantizedKVCacheUsesFusedDecodeQuantizerWhenExplicitlyEnabledForSingleTokenAffineQ4'`: 11 tests passed with code coverage enabled; warnings were pre-existing Swift deprecation/concurrency diagnostics in vendored/runtime files.
- `python3 scripts/swift_changed_line_coverage.py --diff-from origin/main --binary services/mlx-text-worker-swift/.build/arm64-apple-macosx/debug/MelixTextWorkerSwiftPackageTests.xctest/Contents/MacOS/MelixTextWorkerSwiftPackageTests --profdata services/mlx-text-worker-swift/.build/arm64-apple-macosx/debug/codecov/default.profdata third_party/mlx-swift-lm/Libraries/MLXLMCommon/TurboQuantFusedAttention.swift services/mlx-text-worker-swift/Tests/CoreTests/WorkerScaffoldTests.swift`: changed-line coverage `100.00% (82/82)`.
