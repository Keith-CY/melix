...
Keep update-check behavior unchanged while avoiding repeated `UpdateCheckResult` allocation and version comparison work for repeated checks against the same resolved channel metadata and installed version. The channel file stat/cache path remains the source of freshness for `latest_version` and `channel`; this slice only memoizes the immutable final result tuple after that metadata is resolved.
...
1. Add a regression test proving repeated checks reuse the cached result without re-running version comparison.
