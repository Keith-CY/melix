# Local Job JSON Tuple8 Copy Fast Path
...
the existing exact scalar tuple fast paths from two through seven elements to
exact eight-element scalar tuples that can appear in compact local-job follow-up
...
This slice extends the probe payload with a representative exact eight-item
