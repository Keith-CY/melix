...
Tokenizer-owned immutable mask templates are initialized once per tokenizer and accessed through a synchronized cache interface. Shared hits, insert-if-absent writes, eviction, and byte accounting are serialized while full-vocabulary mask computation remains outside the lock. The mutable entry container is not exposed to processors, so concurrent schema and tool requests cannot corrupt LRU order or desynchronize retained-byte accounting.
...
- Required and named tool choices use sampler-enforced Qwen JSON or XML-parameter wire grammars. Named selection cannot enter another tool's prefix, required selection admits only declared tools, parallel calls preserve whitespace separators, and no prose prefix is accepted for non-reasoning routes. Registered parser modes without an executable, parser-round-tripped sampler descriptor fail closed with `tool_wire_parser_mode_unsupported`; they never inherit Qwen markers implicitly.
...
- Gate repeated processor construction so cached tokenizer vocabulary and cached normalized schema compilation avoid unnecessary repeated full-vocabulary decode and schema normalization work. Measure the warm path as a per-build average across a configurable batch while preserving the zero tokenizer-decode assertion per build, so sub-millisecond cache regressions are not decided by a single timer interval.
...
- Importing the real-model evidence script must not load MLX-LM or its native library. The script keeps injectable `load`, `stream_generate`, and `make_sampler` entry points, but imports their MLX-LM implementations only when the real-model probe invokes them. This allows the Linux structured-output gate to collect helper tests without `libmlx.so` while preserving the explicit native dependency for an actual real-model run.
