# Retrieval copy list10 fast path
...
limited to explicit copy fast paths for exactly ten-item JSON-like list and
...
That probe defines focused `test_command`, `coverage_command`, and
`probe_command` entries. This slice adds the plan to the probe `watch_globs` and
extends the existing probe workload with ten-item list and tuple windows so CI
compares `origin/main` against the head implementation using the registered
probe script.

## Plan

1. Add regression coverage proving ten-item lookup payload lists and tuples are
   copied with the same deep-copy semantics as existing short payload windows.
2. Add explicit ten-item list and tuple branches in `_copy_payload_value()`.
3. Extend `scripts/retrieval_context_projection_probe.py` with representative
   ten-item metadata windows.
