...
The probe resolves an older active model ID (`melix-dev-derived-0001`) so the registered metric exercises the cached-ID path instead of mostly measuring the newest-row fast case. The direct lookup and manifest-path lookup timing metrics use a 0.01 ms absolute noise floor because their optimized warm-cache values are intentionally in the low microsecond range; `restore_elapsed_ms_mean` remains the larger, fully gated end-to-end restore signal.
