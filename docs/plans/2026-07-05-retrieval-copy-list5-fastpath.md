# Retrieval lookup list-5 copy fast path

This Python-only performance slice is limited to five-item list handling inside
...
Previous retrieval lookup copy slices added explicit fast paths for short list
and tuple payloads. This slice extends the list path from the existing zero-,
one-, two-, and three-item branches to the five-item metadata score lists used
by the registered projection probe sample.

Behavior remains unchanged:

- list payloads remain lists;
...
- other list lengths continue to use the existing generic copy path.
...
copy sample exercises a five-item list.

## Verification plan

1. Extend focused regression coverage for five-item list payload copies.
