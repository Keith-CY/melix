...
local tool runtime across benchmark, evaluation sample generation, and RL
alignment rollout.
...
2. Preserve tool-call and observation evidence on `EvaluationSample` JSONL
   payloads without changing existing CSV columns.
3. Execute case-level tool calls in benchmark request rows.
4. Execute sample-level tool calls in evaluation sample generation.
5. Execute the same sample-level tool calls in RL policy trace construction.
6. Add focused tests for adapters, benchmark persistence, evaluation
   persistence, and RL trace reuse.
