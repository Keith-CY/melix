# Local Job JSON Tuple9 Copy Fast Path
...
the existing exact scalar tuple fast paths from two through eight elements to
exact nine-element scalar tuples that can appear in compact local-job follow-up
...
This slice extends the probe payload with a representative exact nine-item
