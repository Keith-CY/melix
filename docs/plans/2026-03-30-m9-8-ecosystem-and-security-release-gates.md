...
- [ ] Add typed evidence collectors for MCP auto-injection, agent export smoke, shared-access smoke, persistent-session smoke, sanitization smoke, connection-lifecycle smoke, and closure-audit output.
- [ ] Feed those collectors into the release gate and metrics report with stable field names and deterministic local-fixture data.
- [ ] Add failing and then passing tests for missing-evidence failure, threshold failure, and success with complete M9 evidence.
...
- [ ] Extend the release-gate policy JSON with thresholds for M9 probes and counts, including missing-probe and failed-threshold behavior.
- [ ] Document how M9 ecosystem and security signals influence the release decision and how to interpret failures.
- [ ] Keep the runbook examples synchronized with the actual gate and metrics commands.
...
- [ ] Add a deterministic smoke command that assembles fixture M9 evidence, runs the updated release gate, and emits machine-readable pass or fail output.
- [ ] Add tests that verify the smoke command covers both passing and failing gate states.
- [ ] Record `release_gate.m9_required_probe_count`, `release_gate.m9_missing_probe_count`, and `release_gate.m9_failed_threshold_count`.
...
- [ ] Record the changed-scope metrics report for `release_gate.m9_required_probe_count`, `release_gate.m9_missing_probe_count`, and `release_gate.m9_failed_threshold_count`.
- [ ] Commit Task 8:
