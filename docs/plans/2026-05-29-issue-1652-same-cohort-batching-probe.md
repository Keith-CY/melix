...
5. Register the probe with PR-scoped performance so changes to the script, Swift
   ...
   linked request count, and the non-negative scheduler-to-worker batch gap.
   Because this PR introduces the probe script, the registered command includes
   a missing-script baseline fallback that emits neutral zero evidence plus a
   warning marker when running against `origin/main`; the head probe remains the
   source of the new macOS Swift evidence.
