...
`macos-15` arm64 image used by Melix includes Xcode 26.3, but its default Xcode
is 16.4 and therefore exposes Swift 6.1. Allowing jobs to inherit that default
makes dependency resolution fail before tests start.
...
- `.github/actions/setup-melix-swift-toolchain/action.yml` is the single
  ...
  baseline.
- The action selects `/Applications/Xcode_26.3.app/Contents/Developer` through
...
- the action to report Xcode 26.3 and Apple Swift 6.3.x;
...
If GitHub removes Xcode 26.3 from `macos-15`, update the version, verification
prefix, and cache namespace together in the action, then rerun all affected
