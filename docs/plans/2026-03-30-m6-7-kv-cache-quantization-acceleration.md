...
- Swift worker metrics export `active_kv_runtime_route` and `active_kv_runtime_block_reason` so release-gate evidence can distinguish fallback because the runtime hook is blocked from fallback caused by unsupported cache state or unknown routing
- default Swift runtime now keeps the fused TurboQuant candidate dispatch disabled while the route is blocked; set `MELIX_SWIFT_TURBOQUANT_CANDIDATE_PROBE=1` before `scripts/dev_up.sh` only for candidate-audit runs that need `active_kv_candidate_dispatch_code = 1`
