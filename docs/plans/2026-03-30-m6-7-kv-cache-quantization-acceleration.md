...
- the first runtime candidate dispatches the fused MSE q4 Metal smoke kernel once from `turboquant-q4` decode and reports `active_kv_candidate_dispatch_code = 1` after that dispatch runs; `active_kv_kernel_path` must remain `fallback` until model attention is actually routed through a fused TurboQuant cache, and it is not a success condition unless `worker_tps_overhead_pct <= 15`
...
- inspect `swift_worker_direct.decode[]` for `label = "decode_turboquant_q4"`; `active_kv_candidate_dispatch_code = 1` only proves the candidate smoke dispatch ran, while `active_kv_kernel_path != "fallback"` is required before claiming fused TurboQuant
