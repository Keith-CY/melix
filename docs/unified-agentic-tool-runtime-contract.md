...
| --- | --- | --- | --- | --- | --- |
| M1: define registry and observation contracts | #676 Add tool registry contract | `services/mlx-worker-python/worker/runtime/tool_registry.py`; `services/mlx-worker-python/tests/test_tool_registry.py`; `packages/protocol/schema/worker/v1/common.proto` if protocol fields change; generated protocol outputs if schema changes | focused registry unit tests; `make proto` only if schema changes; `git diff --check` | `agentic_tool_registry.tool_count`; schema bytes; required argument count | adapter execution is M2; benchmark/evaluation routing is M3 |
| M1: define registry and observation contracts | #677 Define observation redaction, limits, timeout, replay metadata | `services/mlx-worker-python/worker/runtime/tool_observation.py`; `services/mlx-worker-python/tests/test_tool_observation.py`; training dataset trace validation when observation shape changes | focused observation unit tests; UTF-8 truncation/redaction tests; changed-scope coverage; `git diff --check` | observation record count; redacted value count; truncated count; timeout count; original/emitted bytes | does not execute adapters by itself |
| M2: implement deterministic local adapters | #679 Implement fixture-backed search/visit/layout tools | new or updated `services/mlx-worker-python/worker/runtime/agentic_tools*.py`; fixture roots under `services/mlx-worker-python/fixtures/`; adapter tests under `services/mlx-worker-python/tests/` | focused adapter tests for `text_search`, `image_search`, `visit`, `layout_parse`, `image_crop`, and `local_compute`; no-network fixture tests; changed-scope coverage | per-tool calls; latency milliseconds; fixture cache hits; emitted bytes | real external providers remain out of scope |
| M2: implement deterministic local adapters | #680 Add timeout, cancellation, and failure-stage tests | adapter runtime module from #679; worker request-state or cancellation helpers only if needed; focused tests | timeout tests; cancellation tests; failed status tests; fatal-stage propagation tests; changed-scope coverage | timeout count; failed count; canceled count; fatal-stage count | does not yet route evaluation or rollout |
| M3: route evaluation and rollout through same tools | #682 Wire tool execution into evaluation sample generation and persisted sample evidence | `services/mlx-worker-python/worker/engine/evaluation_core.py`; `services/mlx-worker-python/worker/productization/evaluation_store.py`; `services/mlx-worker-python/worker/productization/run_evidence.py`; Swift export bridge files only if exported schema changes | focused evaluation tests with fixture-backed tool calls; store/export tests; report evidence tests; changed-scope coverage; persisted artifact smoke when behavior changes | tool-call count per sample; observation count; timeout/failed counts; scoring latency; emitted bytes | benchmark integration can land separately if evaluation evidence lands first |
| M3: route evaluation and rollout through same tools | #683 Add rollout harness that records tool calls and observations | `services/mlx-worker-python/worker/model_ops/rl_alignment_training.py`; rollout harness module if split out; trajectory package fixtures/tests | rollout fixture tests; fatal-aware trajectory tests; reward-component tests; changed-scope coverage | trajectory count; tool turns per trajectory; fatal trace count; reward component means | online GRPO optimization remains separate from the local evidence harness |
