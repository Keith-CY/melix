...
Managed Hugging Face imports keep Melix registry identity separate from model bytes. The standard Hugging Face cache owns immutable snapshot payloads through the normal `huggingface_hub` resolution order (`HUGGINGFACE_HUB_CACHE`, `HF_HOME/hub`, then `~/.cache/huggingface/hub`). `MELIX_MANAGED_MODEL_ROOT` stores only a lightweight descriptor such as `huggingface/<org>/<repo>/<revision>/manifest.json`; that manifest preserves registry identity and sets `melix.model_path` to the resolved Hugging Face snapshot used at runtime. Managed import byte counters describe the runtime snapshot size rather than the descriptor size. If a descriptor points at a missing runtime snapshot, registry metadata includes `melix.model_path_missing=true` so API consumers can distinguish stale descriptors from loadable models. Control-plane, CLI, Desktop, and OpenAI-compatible surfaces treat that metadata as a stable runtime status: the shared error code is `model_runtime_missing`, the shared message is `Hugging Face cache files are missing. Re-download this model to restore it.`, and new load/chat/API requests are rejected before worker dispatch until the user explicitly restores the download. Desktop rows show `Missing cache` and a `Restore Download` action; CLI list output shows `missing-cache` and inspect JSON includes `restore_command`. Local imports remain copied into `MELIX_MANAGED_MODEL_ROOT/local/<model-id>/<revision>`.
