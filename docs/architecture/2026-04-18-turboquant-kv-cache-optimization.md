...
The May 4 re-audit of the same external repos did not change the main route
decision. Current oMLX still keeps fused TurboQuant on the one-token decode
path and leaves batch logic as cache-state management. vMLX has useful
longer-term cache ideas, especially TQ-native disk serialization, codebook
weight caching, and safetensors quantization shape repair, but those do not
directly reduce Melix's already-routed one-token fused decode kernel. The
current Atomic-Chat MLX tree is useful as an operational MLX/server reference,
but it does not expose a directly reusable TurboQuant fused-KV kernel path.
...
The May 4 group-scale shuffle slice keeps the same one-token fused route and
does not claim a fresh real-model throughput gate. It reduces repeated q4
affine scale/bias metadata loads inside each q4 group: for the Qwen3.5-style
`headDimension = 128`, `groupSize = 64` plan, only 2 group-leader lanes load
key/value scale-bias metadata and distribute it with `simd_shuffle`, while the
score reduction still uses 16 active packed-word lanes. Targeted fused-output
tests preserve GQA, bfloat16, and storage-fastpath correctness.
