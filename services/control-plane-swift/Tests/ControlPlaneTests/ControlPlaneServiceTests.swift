...
struct ControlPlaneServiceTests {
    ...
    @Test("execute imports the phase1 baseline repo for direct text evaluation")
    func executeImportsPhase1BaselineTextEvaluationTarget() async throws {
        let repoID = "mlx-community/Qwen3.5-0.8B-OptiQ-4bit"
        let importedEvalClient = ScriptedModelOperationsWorkerClient()
        await importedEvalClient.setHubModelCardResponse(
            makeBenchmarkHubModelCardResponse(
                repoID: repoID,
                modelName: "Qwen3.5-0.8B-OptiQ-4bit",
                pipelineTag: "text-generation",
                tags: ["qwen", "mlx", "text-generation"],
                siblingFiles: [
                    "config.json",
                    "tokenizer.json",
                    "model.safetensors.index.json",
        ...
        await importedEvalClient.setEvaluationResponse({
            var response = Melix_Worker_V1_RunEvaluationResponse()
            response.ok = true
            response.job.jobID = "eval-phase1-qwen"
            response.job.modelID = repoID
            response.job.taskKind = "text-generation"
            response.job.sourceRepo = repoID
            response.job.suiteID = "mmlu"
            response.job.datasetID = "mmlu.dev.v1"
        ...
        #expect(response.ok == true)
        #expect(loadRequest.model.modelID == repoID)
        #expect(loadRequest.model.modelKind == "text")
        #expect(evalRequest.taskKind == "text-generation")
        #expect(evalRequest.sourceRepo == repoID)
        #expect(response.ops.evaluationJob.modelID == repoID)
        #expect(response.ops.evaluationJob.taskKind == "text-generation")
        #expect(response.ops.evaluationJob.sourceRepo == repoID)
    }

    @Test("execute surfaces phase1 direct evaluation worker failures")
    func executeSurfacesPhase1DirectEvaluationWorkerFailures() async throws {
        let repoID = "mlx-community/Qwen3.5-0.8B-OptiQ-4bit"
        let importedEvalClient = ScriptedModelOperationsWorkerClient()
        await importedEvalClient.setHubModelCardResponse(
            makeBenchmarkHubModelCardResponse(
        ...
        await importedEvalClient.setEvaluationError(WorkerClientError.unavailable)
        let importedRuntimeClient = ScriptedChatWorkerClient(events: [])
        let service = ControlPlaneService(
            modelCatalog: ModelCatalog(seedModels: ModelCatalog.phaseSevenContractSeedModels()),
            workerRegistry: WorkerRegistry(
        ...
        #expect(response.ok == false)
        #expect(response.error.code == "unavailable")
        #expect(response.error.message.contains("Evaluation worker request failed"))
    }
