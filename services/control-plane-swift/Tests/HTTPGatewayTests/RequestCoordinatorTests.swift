...
struct RequestCoordinatorTests {
    ...
    func gatewaySpeculativeDefaultsDowngradeUnsupportedModelPolicies() async throws {
        ...
        #expect(vlmDefaults.maxConcurrentRequests == 3)
        ...
        #expect(vlmDefaults.accelerationProfile == "")
        #expect(vlmDefaults.overrideReceiptExt["melix.gateway.suppressed_overrides"] == "max_concurrent_requests,speculative_decode")
        #expect(vlmDefaults.overrideReceiptExt["melix.gateway.batch.disabled_reason"] == "incompatible_batch_size")
        #expect(vlmDefaults.overrideReceiptExt["melix.gateway.speculative.disabled_reason"] == "unsupported_route")

        let textDefaults = speculativeDefaults.resolvingAccelerationCompatibility(for: textModel)
        #expect(textDefaults.accelerationMode == .speculativeDecode)
        #expect(textDefaults.draftModelID == "melix-dev-text")
        #expect(textDefaults.numDraftTokens == 7)
        #expect(textDefaults.maxConcurrentRequests == 3)
        #expect(textDefaults.prefillBatchSize == 3)
        #expect(textDefaults.completionBatchSize == 3)
        #expect(textDefaults.overrideReceiptExt["melix.gateway.suppressed_overrides"] == "max_concurrent_requests")
        #expect(textDefaults.overrideReceiptExt["melix.gateway.effective_speculative_mode"] == "speculative_decode")
    }
