...
This is the integration point that lets Melix's VLM text-only
``BatchGenerator`` path drive MTP without replacing mlx-lm's scheduler.
``GenerationBatch`` is mlx-lm's per-step decoder for the active set of
sequences in continuous batching. We patch:
...
encodes this contract; the Melix-side equivalent lives in
...
``GatedDeltaNet.__call__`` and is restored on reject. Because both code
  paths only mutate cache *length* (not block ownership), so cache ownership
  remains with mlx-lm's standard batch generator.
...
def apply() -> bool:
    ...
    if hasattr(GenerationBatch, "_melix_mtp_patched"):
        _PATCHED = True
        return True
    ...
    def patched_extend(self, batch, *args, **kwargs):
        ...
        host_state = getattr(self, "_melix_mtp_state", None)
...
def _is_mtp_eligible(gen_batch: Any) -> bool:
    ...
    reflects the per-load ``melix.native_mtp.enabled`` choice.
    """
    ...
    if not _model_has_mtp_module(gen_batch.model):
        return False
    if not _is_model_mtp_active(gen_batch.model):
        return False
...
def _ineligibility_reason(gen_batch: Any) -> str:
    ...
    if not _is_model_mtp_active(gen_batch.model):
        return "native MTP is inactive for this loaded model"
    uids = getattr(gen_batch, "uids", None)
    if uids is None:
...
def _is_model_mtp_active(model: Any) -> bool:
    per_model = getattr(model, "_melix_native_mtp_active", None)
    if per_model is not None:
        return bool(per_model)
    inner = getattr(model, "language_model", model)
    per_inner_model = getattr(inner, "_melix_native_mtp_active", None)
    if per_inner_model is not None:
        return bool(per_inner_model)
    try:
...
class _MtpState:
    ...
    # adapted to Melix's callable-sampler contract via metadata-introspection.
...
def _has_grammar_processors(gen_batch: Any) -> bool:
    ...
    if not processors_by_seq:
        return False
    return any(
        proc.__class__.__name__ == "GrammarConstraintProcessor"
...
def _drop_mtp_state(
    ...
    *,
    ...
    state = getattr(gen_batch, "_melix_mtp_state", None)
    if state is None:
        return None
    if log_stats:
        try:
            ...
            pass
    try:
        delattr(gen_batch, "_melix_mtp_state")
...
def _accept_lp_for(sampler, lp):
    ...
    ``mlx_lm.sample_utils.make_sampler``). For samplers without metadata —
...
def _post_init_mtp(gen_batch: Any) -> None:
    ...
    gen_batch._melix_mtp_state = state
...
def _step_mtp(
    ...
    ``gen_batch._melix_mtp_state.draft_id`` so the next verify cycle's
    ...
    state = gen_batch._melix_mtp_state
    sampler = _resolve_sampler(gen_batch)
    procs = _proc_list(gen_batch)
...
def _emit_response(
    ...
    if finish_reason is not None:
        ...
        _attach_mtp_response_stats(response, stats, terminal=True)
        if stats is not None:
        ...
        if hasattr(gen_batch, "_melix_mtp_state"):
            try:
                delattr(gen_batch, "_melix_mtp_state")
            except AttributeError:
                pass
    ...
    _attach_mtp_response_stats(response, stats, terminal=False)
    return [response]


def _attach_mtp_response_stats(
    response: Any,
    stats: Optional["_MtpStats"],
    *,
    terminal: bool,
) -> None:
    if stats is None or not terminal:
        return
    attempted = int(stats.accepts + stats.rejects)
    response.speculative_acceptance_rate = (
        float(stats.accepts / attempted) if attempted > 0 else None
    )
    response.speculative_rollback_rate = (
        float(stats.rejects / attempted) if attempted > 0 else None
    )
    response.speculative_accepted_tokens = int(stats.accepts)
    response.speculative_rejected_tokens = int(stats.rejects)
    response.speculative_num_draft_tokens = 1
    response.speculative_draft_model_configured = True
    response.speculative_cycle_count = int(stats.cycles)
    response.speculative_backbone_ms = float(stats.backbone_ms)
    response.speculative_mtp_head_ms = float(stats.mtp_head_ms)
    response.speculative_sample_ms = float(stats.sample_ms)
    response.speculative_cache_ops_ms = float(stats.cache_ops_ms)
