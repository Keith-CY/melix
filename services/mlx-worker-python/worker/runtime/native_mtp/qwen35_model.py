...
the Melix runtime patch package: replace class methods on a one-shot,
...
``mlx_vlm.models.qwen3_5.language`` is a separate copy patched by
``qwen35_vlm_runtime.py``.
"""
...
def apply() -> bool:
    ...
    if hasattr(q35.TextModel, "mtp_forward") and not hasattr(
        q35.TextModel, "_melix_mtp_patched"
    ):
        _PATCHED = True
        q35.TextModel._melix_mtp_patched = "upstream"
        return True
    ...
    _PATCHED = True
    q35.TextModel._melix_mtp_patched = "patch"
...
def _patch_text_model_args(q35: Any) -> None:
    ...
    if hasattr(args_cls, "_melix_mtp_from_dict_patched"):
        return
    ...
    args_cls._melix_mtp_from_dict_patched = True
...
def _patch_gated_delta_net(q35: Any) -> None:
    ...
    if "_melix_mtp_patched" in cls.__dict__:
        return
...
def _patch_qwen3_5_moe() -> None:
    ...
    def _stack_per_expert(weights, prefix, num_experts):
        ...
        # so Melix-quantized MoE MTP layers load correctly. Without this, only
        ...
        if mtp_num > 0:
            ...
            if not has_any_mtp:
                ...
                for layer_idx in range(mtp_num):
                    ...
                    # Idempotent: converted outputs may already store experts in
