...
class DeterministicEmbeddingBackend:
    ...
    def _project_digest(
        ...
        *,
        ...
        if dimensions < 0:
            return []
        digest = _sha256(seed_text.encode("utf-8")).digest()
        if dimensions == 1:
            value = _UNPACK_DIGEST_FIRST_UINT32(digest)[0] * _DIGEST_UINT32_SCALE - 1.0
            if value > 0.0:
                return [1.0]
            if value < 0.0:
                return [-1.0]
            return [0.0]
        digest_words = _UNPACK_DIGEST_UINT32(digest)
