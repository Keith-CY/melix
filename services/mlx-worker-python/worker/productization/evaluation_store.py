...
class EvaluationStore:
    ...
    def _compare_summary_csv_row(
        *,
        ...
        return ",".join(
            [
                csv_field(job.job_id),
                csv_field(job.base_model_id),
                csv_field(summary.target_model_id),
                csv_field(job.suite_id),
                csv_field(job.dataset_id),
                csv_field(str(job.sample_size)),
                csv_field(str(summary.win_count)),
                csv_field(str(summary.loss_count)),
                csv_field(str(summary.tie_count)),
                csv_field(str(summary.regression_count)),
                csv_field(str(summary.base_accuracy)),
                csv_field(str(summary.target_accuracy)),
                csv_field(str(summary.delta_accuracy)),
                csv_field(str(summary.effect_threshold)),
                csv_field(summary.verdict),
                csv_field(str(bootstrap_interval.get("lower_bound", ""))),
                csv_field(str(bootstrap_interval.get("upper_bound", ""))),
                csv_field(str(analytical_interval.get("lower_bound", ""))),
                csv_field(str(analytical_interval.get("upper_bound", ""))),
                csv_field(str(summary.duration_seconds)),
                csv_field(str(job.created_at_unix_ms)),
                csv_field(lineage.adapter_manifest_path if lineage else ""),
                csv_field(lineage.adapter_set_hash if lineage else ""),
            ]
        )
