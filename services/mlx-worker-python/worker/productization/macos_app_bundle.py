    ...
    return "\n".join(
        [
            ...
            *packaged_python_import_isolation_env_exports(),  # pragma: no cover
            ...
            '  for _ in {1..40}; do',
            '    if ! kill -0 "$private_pid" >/dev/null 2>&1; then',
            '      wait "$private_pid" >/dev/null 2>&1 || true',
            '      return 0',
            '    fi',
            '    private_process_state="$(/bin/ps -o stat= -p "$private_pid" 2>/dev/null || true)"',
            '    if [[ -z "$private_process_state" || "$private_process_state" == *Z* ]]; then',
            '      wait "$private_pid" >/dev/null 2>&1 || true',
            '      return 0',
            '    fi',
            '    /bin/sleep 0.05',
