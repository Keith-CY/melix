...
def test_managed_hub_import_honors_explicit_hf_cache_root(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    pipeline = DownloadPipeline()
    requested_cache_root = tmp_path / "custom-hf-cache"
    snapshot_dir = requested_cache_root / "models--mlx-community--demo" / "snapshots" / "main"
    ...
    request = maintenance_pb2.ConvertModelRequest(
        ...
        ext={
            ...
            "melix.hf_repo_id": "mlx-community/demo",
            "melix.hf_revision": "main",
            "melix.hf_cache_root": str(requested_cache_root),
        },
    )

    result = pipeline.run(request, job_id="job-custom-cache", output_dir=tmp_path / "output")

    payload = json.loads(result.snapshots[-1].manifest_json)
    assert observed_kwargs["cache_dir"] == str(requested_cache_root.resolve())
    assert result.output_path == snapshot_dir.resolve()
    assert payload["managed_model_path"] == str(snapshot_dir.resolve())
    assert payload["ext"]["melix.hf_cache_root"] == str(requested_cache_root)
    assert payload["ext"]["melix.effective_hf_cache_root"] == str(requested_cache_root.resolve())


def test_managed_hub_import_honors_hf_home_environment(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    pipeline = DownloadPipeline()
    hf_home = tmp_path / "hf-home"
    snapshot_dir = hf_home / "hub" / "models--mlx-community--demo" / "snapshots" / "main"
    snapshot_dir.mkdir(parents=True)
    (snapshot_dir / "config.json").write_text("{}", encoding="utf-8")
    (snapshot_dir / "weights.safetensors").write_bytes(b"weights")
    observed_kwargs: dict[str, object] = {}
    ...
    result = pipeline.run(request, job_id="job-hf-home", output_dir=tmp_path / "output")

    payload = json.loads(result.snapshots[-1].manifest_json)
    assert observed_kwargs["cache_dir"] == str((hf_home / "hub").resolve())
    assert result.output_path == snapshot_dir.resolve()
...
def test_managed_hub_cache_root_honors_request_hf_home(tmp_path: Path) -> None:
    ...
    assert DownloadPipeline._huggingface_cache_root({"HF_HOME": str(hf_home)}) == (hf_home / "hub").resolve()


def test_managed_hub_cache_root_honors_huggingface_hub_cache_environment(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    cache_root = tmp_path / "env-hf-cache"
