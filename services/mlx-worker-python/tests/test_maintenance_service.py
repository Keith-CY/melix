...
def test_percentiles_reuse_one_sorted_vector_and_preserve_interpolation(
    ...
    assert MaintenanceCore._percentile(values, 0.0) == 1.0
    assert MaintenanceCore._percentile(values, 100.0) == 9.0
    assert sorted_calls == [
        [1.0, 5.0, 7.0, 9.0],
        [1.0, 5.0, 7.0, 9.0],
        [1.0, 5.0, 7.0, 9.0],
...
def test_benchmark_helper_parsers_cover_invalid_and_boundary_inputs(
    ...
    assert shaped_repeated_prompt.split() is split_tokens
