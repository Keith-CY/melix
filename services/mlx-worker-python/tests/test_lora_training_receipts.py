...
def test_lora_training_pipeline_records_skipped_heldout_evaluation_without_test_ratio(
    ...
    assert manifest["dataset_files_resolved"]["normalized_test_path"] == ""
    assert receipt == {
        "schema_version": "melix.lora_heldout_evaluation_receipt.v1",
        "status": "skipped",
...
def test_lora_canary_receipt_records_tokenizer_resume_and_drift_status(
    ...
    test_lora_training_pipeline_records_completed_heldout_evaluation_receipt(
        tmp_path / "heldout-completed"
    )
    test_lora_training_pipeline_records_skipped_heldout_evaluation_without_test_ratio(
        tmp_path / "heldout-skipped"
    )
