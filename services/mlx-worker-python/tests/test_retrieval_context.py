...
def test_retrieval_lookup_payload_copy_preserves_scalar_and_none_values() -> None:
    ...
    assert (
        copied["retrieved_context"]["metadata"]["seven_labels"]
        == payload["retrieved_context"]["metadata"]["seven_labels"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["seven_labels"]
        is not payload["retrieved_context"]["metadata"]["seven_labels"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["seven_labels"][1]
        is not payload["retrieved_context"]["metadata"]["seven_labels"][1]
    )
    assert (
        copied["retrieved_context"]["metadata"]["eight_labels"]
        == payload["retrieved_context"]["metadata"]["eight_labels"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["eight_labels"]
        is not payload["retrieved_context"]["metadata"]["eight_labels"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["eight_labels"][1]
    ...
    assert (
        copied["retrieved_context"]["metadata"]["twelve_labels"]
        == payload["retrieved_context"]["metadata"]["twelve_labels"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["twelve_labels"]
        is not payload["retrieved_context"]["metadata"]["twelve_labels"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["twelve_labels"][1]
        is not payload["retrieved_context"]["metadata"]["twelve_labels"][1]
    )
    assert (
    ...
    assert (
        copied["retrieved_context"]["metadata"]["seven_scores"]
        == payload["retrieved_context"]["metadata"]["seven_scores"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["seven_scores"]
        is not payload["retrieved_context"]["metadata"]["seven_scores"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["seven_scores"][2]
        is not payload["retrieved_context"]["metadata"]["seven_scores"][2]
    )
    assert (
        copied["retrieved_context"]["metadata"]["eight_scores"]
        == payload["retrieved_context"]["metadata"]["eight_scores"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["eight_scores"]
        is not payload["retrieved_context"]["metadata"]["eight_scores"]
    )
    assert (
    ...
    assert (
        copied["retrieved_context"]["metadata"]["twelve_scores"]
        == payload["retrieved_context"]["metadata"]["twelve_scores"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["twelve_scores"]
        is not payload["retrieved_context"]["metadata"]["twelve_scores"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["twelve_scores"][2]
        is not payload["retrieved_context"]["metadata"]["twelve_scores"][2]
    )
    assert (
        copied["retrieved_context"]["metadata"]["long_scores"]
        == payload["retrieved_context"]["metadata"]["long_scores"]
    )
    assert (
        copied["retrieved_context"]["metadata"]["long_scores"]
        is not payload["retrieved_context"]["metadata"]["long_scores"]
    )
    assert (
...
def test_project_retrieval_lookup_result_uses_wrapper_metadata_for_refusals() -> None:
    projection = project_retrieval_lookup_result(
        {},
        lookup_source_id=" live-rag:search-7 ",
        lookup_segment_id=" live-rag:search-7:lookup ",
    ...
    assert projection.refusal_receipts == [
        {
            "schema_version": "melix.untrusted_context_receipt.v1",
            "segment_id": "live-rag:search-7:lookup",
            "source_type": "retrieval_lookup",
            "source_field": "live_rag_records",
            "source_id": "live-rag:search-7",
            "message_role": "user",
            "trust_level": "untrusted",
            "policy": "data_only",
            "boundary_checked": True,
    ...
    malformed = project_retrieval_lookup_result(
        "bad-wrapper",
        lookup_source_id="live-rag:search-7",
        lookup_segment_id="live-rag:search-7:lookup",
        lookup_source_field="live_rag_lookup",
    )

    assert malformed.prompt_user_payload == {}
    assert malformed.untrusted_context_receipts == []
    assert malformed.lookup_message is None
    assert malformed.refusal_receipts[0]["segment_id"] == "live-rag:search-7:lookup"
    assert malformed.refusal_receipts[0]["source_type"] == "retrieval_lookup"
    assert malformed.refusal_receipts[0]["source_field"] == "live_rag_lookup"
