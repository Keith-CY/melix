...
def build_empty_streaming_services():
    registry = WorkerRegistry(
        runtime=MLXTextRuntime(backend=EmptyStreamingFakeBackend()),
        model_catalog=WorkerModelCatalog(),
    )
    runtime_service = WorkerRuntimeService(registry)
    inference_service = WorkerInferenceService(registry)
...
def test_generate_empty_completion_skips_empty_parser_metric_assembly() -> None:
    _, inference_service, model_handle = build_empty_streaming_services()
    ...
    request = inference_pb2.GenerateRequest(
        ...
        messages=[
            common_pb2.ChatMessage(
                role="user",
                parts=[common_pb2.MessagePart(text="Say nothing")],
            )
        ],
    ...
    assert completed.parser_metrics["turn_boundary_stop_reason"] == "stop"
    assert not any(event.HasField("token_delta") for event in events)
    assert not any(event.HasField("usage_delta") for event in events)
