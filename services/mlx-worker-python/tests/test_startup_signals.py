...
def test_read_product_version_reuses_absolute_string_without_resolve(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    (tmp_path / "pyproject.toml").write_text(
    ...
    def fail_resolve(
        ...
        **kwargs: object,
        ...
        raise AssertionError(f"read_product_version should not resolve absolute string inputs: {self}")

    monkeypatch.setattr(Path, "resolve", fail_resolve)

    assert read_product_version(str(tmp_path)) == "1.2.3"


def test_read_product_version_keeps_relative_string_resolution(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    (tmp_path / "pyproject.toml").write_text(
        '[project]\nname = "melix"\nversion = "1.2.3"\n',
        encoding="utf-8",
    )
    ...
    assert read_product_version(tmp_path.name) == "1.2.3"
